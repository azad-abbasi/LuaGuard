...
function
 : Function names ( Col Name {addSelf=true;} func_body {addSelf=false;} 
                    -> ^(FUNCTION_ASSIGNMENT ^(VAR_LIST {namesToVar($names.list, $Name.text)}) ^(EXPR_LIST func_body))
                  | func_body
                    -> ^(FUNCTION_ASSIGNMENT ^(VAR_LIST {namesToVar($names.list)}) ^(EXPR_LIST func_body))
                  )
 ;
